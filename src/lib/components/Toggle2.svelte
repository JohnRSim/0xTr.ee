...
<div
	...
	id="{inputname}"
	on:click="{() => {
		dispatchEvent({ action: 'toggle', val: !inputVal });
	}}">
	<input type="checkbox" id="{inputname}_toggle" bind:checked="{inputVal}" class="{inputname}" />
	<label
		for="{disableClick ? undefined : `${inputname}_toggle`}"
		class="toggle"
		data-before="{labelOff}"
		data-after="{labelOn}"><span class="toggle__handler"></span></label>
</div>
